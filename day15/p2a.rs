...
fn get_max_value(remaining: i64, num_seen: usize, stats: &Vec<(i64, i64, i64, i64, i64)>, cur_capacity: i64, cur_durability: i64, cur_flavor: i64, cur_texture: i64, cur_calories: i64) -> Option<i64> {
    if num_seen >= stats.len() {
        if cur_calories == 500 {
            Some(max(0, cur_capacity) * max(0, cur_durability) * max(0, cur_flavor) * max(0, cur_texture))
        } else {
            None
        }
    } else {
        stats.iter().skip(num_seen).map(|&(capacity, durability, flavor, texture, calories)| {
            let mut max_value = None;
            for amt in 0..remaining + 1 {
                if let Some(value) = get_max_value(remaining - amt, num_seen + 1, stats,
                                                   cur_capacity + amt * capacity,
                                                   cur_durability + amt * durability,
                                                   cur_flavor + amt * flavor,
                                                   cur_texture + amt * texture,
                                                   cur_calories + amt * calories) {
                    if !max_value.is_some() || value > max_value.unwrap() {
...
fn main() {
    ...
    let max_value = get_max_value(100, 0, &ingredient_stats.values().map(|t| *t).collect(), 0, 0, 0, 0, 0);
