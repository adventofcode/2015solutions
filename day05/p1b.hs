...
import Data.List (findIndices, transpose, sort, group, groupBy, isPrefixOf, isInfixOf, subsequences, tails)
...
nPairs n str = not . null . filter ((>= n) . length . map dropHalf . groupBy adjacent . snd) . map (id &&& (`substrIndices` str)) . uniques . windows 2 $ str
  where
...
hasPalindrome3 str = or . map (\ str -> str == reverse str) . windows 3 $ str
...
main = interact $ show . countNice niceFunctions1 . lines
