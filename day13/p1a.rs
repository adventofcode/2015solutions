...
fn main() {
    ...
    permutations(&mut happiness_delta.keys().collect::<Vec<&String>>(), |ordering| {
        ...
        let total_value = ordering.iter().zip(ordering.iter().skip(1)).map(|(a, b)| {
            happiness_delta.get(*a).unwrap().get(*b).unwrap() +
                happiness_delta.get(*b).unwrap().get(*a).unwrap()
        }).sum::<i64>() +
            happiness_delta.get(ordering[ordering.len() - 1]).unwrap().get(ordering[0]).unwrap() +
            happiness_delta.get(ordering[0]).unwrap().get(ordering[ordering.len() - 1]).unwrap();
