...
fn main() {
    ...
    permutations(cur_id, |ids| {
        ...
        match max_len_seen {
            None => max_len_seen = Some(current_len),
            Some(n) => if current_len > n {
                max_len_seen = Some(current_len)
            },
        }
    });

    println!("Maximum: {}", max_len_seen.expect("something should be visited"));
}
