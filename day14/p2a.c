...
void MaxDistanceAfterTime(struct reindeer **, int, long);
long MaxPoints(struct reindeer **, int);
...
int main(void)
{
	...
	for (i = 0; i < NUM_SECS; i++) {
		MaxDistanceAfterTime(headlights, numDeer, i+1);
	}
	printf("Max points: %ld\n", MaxPoints(headlights, numDeer));
	return 0;
}
...
void MaxDistanceAfterTime(struct reindeer **headlights,
			int numDeer, long secs)
{
	long retVal = 0;
	long tmp;
	int i;
...
long MaxPoints(struct reindeer **headlights, int numDeer)
{
	long retVal = 0;
	int i;
	for (i = 0; i < numDeer; i++) {
		retVal = retVal > headlights[i]->points ?
				retVal :
				headlights[i]->points;
	}
	return retVal;
